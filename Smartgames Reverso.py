...
def PIECE(N, P,Q):
  ...
  if N == 6: return ((Px,Py,Pz),(Qx,Qy,Qz),(Px,Py,-Pz),(Qx,Qy,-Qz))
  if N == 5: return ((Px,Py,Pz),(Qx,Qy,Qz),(Px,Py,-Pz),(Qx,Qy,-Qz),
                     (Qx+Vx,Qy+Vy,-Qz)) 
  if N == 4: return ((Px,Py,Pz),(Qx,Qy,Qz),(Qx,Qy,-Qz),(Px,Py,-Pz),
                     (Px+Wx,Py+Wy,-Pz))
  if N == 3: return ((Px,Py,Pz),(Qx,Qy,Qz),(Px,Py,-Pz),(Qx,Qy,-Qz),
                     (Qx+Wx,Qy+Wy,-Qz))
  if N == 2: return ((Px,Py,Pz),(Qx,Qy,Qz),(Px,Py,-Pz),(Px+Wx,Py+Wy,-Pz),
                     (Qx+Wx,Qy+Wy,-Qz),(Qx,Qy,-Qz))
  if N == 1: return ((Px,Py,Pz),(Qx,Qy,Qz),(Px,Py,-Pz),(Qx,Qy,-Qz),
...
def DRAW(PIECES):
  ...
  if len(PIECES) > 6:
    for x,y,z in PIECES[6][:2]:
      T[2*y+1][4*x+10*z+12] = "●"
  else:
    for x,y,z in PIECES[1][:2]:
      T[2*y+1][4*x+10*z+12] = "●"
...
def SEARCH(PIECES=[((1,1,1),)]):
  
  if len(PIECES) == 7: 
    if (PIECES[6][0][2] == 1 and PIECES[6][0][0] <= PIECES[6][1][0] 
                             and PIECES[6][0][1] <= PIECES[6][1][1]):
      yield (((PIECES[6][0][0], PIECES[6][0][1], PIECES[6][0][2]), 
              (PIECES[6][1][0], PIECES[6][1][1], PIECES[6][0][2]),
              (PIECES[6][0][0], PIECES[6][0][1],-PIECES[6][0][2]), 
              (PIECES[6][1][0], PIECES[6][1][1],-PIECES[6][0][2])),
              tuple(PIECES))
...
if __name__ == "__main__":
  ...
  for KEY,PIECES in SEARCH():
    SOLUTIONS[KEY].append(PIECES)
